...
The KV-stationary dataflow eliminates score-matrix DRAM by construction and
delivers up to $\mathbf{1.76\times}$ per-MAC advantage over fused FlashAttention
in the compute-bound regime using merge extensions and interleaved MACs.
At commodity 512\,B/cycle both architectures are DRAM-bound with identical
traffic, collapsing the advantage to ${\sim}1\times$ --- the memory wall, not
compute efficiency, is the binding constraint.
The actionable design rules are: size $lmc$ to bandwidth ($lmc{\approx}
\text{BW}/128$); deploy merge extensions only when compute-bound; and use the
multi-pass re-use model to reduce on-chip SRAM by $P\times$ at a sub-linear
latency cost (Appendix~\ref{sec:future}).

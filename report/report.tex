...
($2{\times}H{\times}T^2{\times}\text{bpe} = 17.18\,\text{GB}$: written to DRAM after
GEMM$_1$, re-read for GEMM$_2$), bringing total unfused DRAM to 17.45\,GB of which
the score matrix is 98.4\%.
...
bound. Errors are large for $n{=}0$, $lmc{=}16$ (flat array, short drain) but
converge below 5\% for $n{=}3$ or $lmc{=}1$ at $T{\ge}2048$, where computation
dominates the drain overhead.}
...
$T{-}1$ to $T/2^n{-}1$ column traversals --- effective only in the
compute-bound regime (\S\ref{sec:wavefront}), where drain is the binding term;
inert when DRAM-bound, as both $n{=}0$ and $n{=}3$ pin to the same 273\,MB floor.

...
def kv_stationary_metrics(
    ...
    if num_passes <= 1:
        result = simulate_2d_kv_stationary_array(
            ...
            wavefront_fill=wavefront_fill,
        )
        result.update({
            "num_passes": 1,
            "physical_cols": T,                          # tokens this (only) pass
            "sub_array_cols": array_cols,                # physical columns / sub-array
            "merge_rows": 2 ** merge_extensions,
            ...
            "causal_compute_per_pass": [result["compute_cycles"]],
            "causal_memory_per_pass": [result["memory_service_cycles"]],
            "causal_compute_total": result["compute_cycles"],
            "causal_memory_total": result["memory_service_cycles"],
    ...
    # ── Merge-extension geometry ──────────────────────────────────────────────
    # merge_extensions=n splits each pass's C tokens across 2^n sub-arrays stacked
    # in the row dimension (array_rows = H·2^n) so each sub-array spans only
    # C/2^n columns.  This shortens the pipeline drain (∝ sub-array columns) at
    # the cost of more rows — it does NOT change the total PE/MAC count:
    #     pe_count = array_rows × sub_cols = (H·2^n) × (C/2^n) = H·C
    # For n=0 (merge_rows=1) sub_cols == C and this reduces to the base case.
    merge_rows = 2 ** merge_extensions
    sub_cols   = max(1, C // merge_rows)

    def _pass_sim(qt: int) -> Dict[str, object]:
        """One pass over C tokens (sub_cols physical columns/sub-array), qt queries."""
    ...
    # ── Non-causal single-pass baseline (all query_tokens over one C-token tile) ─
    single_pass = _pass_sim(query_tokens)
    ...
    result.update({
        ...
        "causal": causal,
        "pe_count": array_rows * sub_cols,          # = H·C, merge-invariant
        "sram_bytes": 2 * C * d * bytes_per_element,
    })

    # ── Causal per-pass computation ───────────────────────────────────────────
    # Pass k covers KV columns [(k−1)·C .. k·C−1] with Q_k = T − (k−1)·C active
    # queries.  Compute is delegated to the base simulator with the correct merge
    # geometry (so drain shrinks with 2^n automatically); DRAM is sized to the
    # active queries (only Q_k reload Q; the C diagonal queries write output).
    causal_pass_cycles: List[int] = []
    causal_compute_per_pass: List[int] = []
    causal_memory_per_pass: List[int] = []
    for k in range(1, num_passes + 1):
        Q_k = max(0, T - (k - 1) * C)

        compute_k = _pass_sim(max(1, Q_k))["compute_cycles"]
        causal_compute_per_pass.append(compute_k)
    ...
    result.update({
        ...
        "causal_compute_per_pass": causal_compute_per_pass,
        "causal_memory_per_pass": causal_memory_per_pass,
        "causal_compute_total": sum(causal_compute_per_pass),
        "causal_memory_total": sum(causal_memory_per_pass),

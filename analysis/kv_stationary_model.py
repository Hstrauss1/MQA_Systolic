...
def simulate_2d_kv_stationary_array(
    ...
    pipeline_latency_cycles = (
        d + pipeline_depth_steps * effective_stagger
        if wavefront_fill
        else pipeline_depth_steps * column_dwell
    )
    ...
    for tile_idx in range(token_tiles):
        ...
        # Two pipeline-fill models (selected by wavefront_fill):
        #
        # A — dwell fill (wavefront_fill=False, legacy):
        #   (active_query_rows + active_cols - 1) × column_dwell
        #   Assumes a single packet completes its full d-cycle dot product AT each
        #   column before advancing — Q hops one column every column_dwell (135).
        #
        # B — wavefront fill (wavefront_fill=True, default):
        #   d + (active_query_rows + active_cols - 1) × effective_stagger
        #   Every column owns a dedicated lower MAC + stationary K, so for one packet
        #   the C dot products are INDEPENDENT and compute in parallel as Q ripples
        #   right (1-cycle register hop).  The d-cycle dot-product latency is paid
        #   ONCE as fill; thereafter the binding path is the online-softmax state
        #   chain, advancing one (row|col) step every effective_stagger cycles.
        #   lmc=16 makes effective_stagger=8, so the fill no longer pays 135/column.
        #
        # Both models share the same steady-state stream term (max_packets-1)×eff_stag.
        tile_total_steps  = fill_steps + steady_steps + flush_steps + vertical_fill_steps_per_tile
        pipeline_depth = active_query_rows + active_cols - 1
        if wavefront_fill:
            tile_compute_cycles = (
                d
                + pipeline_depth * effective_stagger
                + (max_packets_per_row - 1) * effective_stagger
            )
        else:
            tile_compute_cycles = (
                pipeline_depth * column_dwell
                + (max_packets_per_row - 1) * effective_stagger
            )
